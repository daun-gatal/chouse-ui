# ADR 0002 — Personal Access Tokens (PAT)

- **Status:** Proposed
- **Date:** 2026-06-16
- **Deciders:** CHouse UI maintainers
- **Tags:** RBAC, auth, credentials, security
- **Related:** [ADR 0003 — ClickHouse Query Gateway](./0003-clickhouse-query-gateway.md) (depends on this), `docs/sso.md`, `docs/datagrip-connection.md`
...
> [ADR 0003](./0003-clickhouse-query-gateway.md), which **depends on** this one.
...
1. **Bound to exactly one connection.** Every PAT targets a single ClickHouse
   ...
   and is a security boundary (see ADR 0003 for why this also removes all
...
- **Hashing choice:** the secret is high-entropy, so **SHA-256 + constant-time
  ...
  consumer (the gateway, ADR 0003) verifies a token on **every request** (hot
...
- `last_used_at` is updated on successful use (throttled write), but **routine
  ...
  in ADR 0003) to avoid double-logging every query here.
...
consumed by ADR 0003, **not** a public endpoint.
...
- **Positive:** a reusable, governed credential unblocking ADR 0003 and any future
...
2. **Per-connection ClickHouse-native passwords.** Rejected here: that's the
   "replicate users into ClickHouse" model ADR 0003 explicitly avoids.
