...
import { pgTable, text, boolean, timestamp, integer, real, uniqueIndex, index, jsonb, varchar } from 'drizzle-orm/pg-core';
...
// is currently DISABLED; savedGrants holds the snapshot to restore on enable.
// ============================================

export const clickhouseRoleState = pgTable('rbac_clickhouse_role_state', {
  id: text('id').primaryKey(),
  connectionId: text('connection_id').notNull().references(() => clickhouseConnections.id, { onDelete: 'cascade' }),
  roleName: varchar('role_name', { length: 255 }).notNull(),
  savedGrants: jsonb('saved_grants').$type<unknown[]>().notNull().default([]),
...
export const aiConfigPolicies = pgTable('rbac_ai_config_policies', {
  id: text('id').primaryKey(),
  configId: text('config_id').notNull().references(() => aiConfigs.id, { onDelete: 'cascade' }),
  capabilityId: varchar('capability_id', { length: 80 }).notNull(),
  ...
  providerOptions: jsonb('provider_options').$type<Record<string, unknown> | null>(),
  fallbackConfigIds: jsonb('fallback_config_ids').$type<string[] | null>(),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
 ...
 */
export const aiChatThreads = pgTable('rbac_ai_chat_threads', {
  id: text('id').primaryKey(),
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  title: text('title'),
  connectionId: text('connection_id').references(() => clickhouseConnections.id, { onDelete: 'set null' }),
 ...
 */
export const aiChatMessages = pgTable('rbac_ai_chat_messages', {
  id: text('id').primaryKey(),
