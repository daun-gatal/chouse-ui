...
export function registerCoreTools(mcp: McpServer, deps: McpDeps): void {
  ...
  mcp.registerTool(
    ...
    async (args: { connection_id: string }, extra) => {
      const ctx = toolContext(extra);
      return runApiTool(ctx, deps.clientFor(ctx), "use_connection", args.connection_id, () =>
        deps.clientFor(ctx).request(
          "GET",
          `/api/rbac/connections/${encodeURIComponent(args.connection_id)}`
        )
      );
    }
  );
