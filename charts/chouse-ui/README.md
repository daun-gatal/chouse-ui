...
| mcp.ingress.enabled | bool | `false` | Expose MCP at `<host>/mcp` through the ingress (TLS-terminated on port 80/443), so agents can use `https://<host>/mcp` without a port. Requires `mcp.enabled` (the path targets the dedicated MCP Service and 503s without it). Note: MCP streams over SSE, so the ingress needs the same annotations as AI chat — raise the proxy read timeout (nginx: `nginx.ingress.kubernetes.io/proxy-read-timeout: "300"`) and disable buffering (`nginx.ingress.kubernetes.io/proxy-buffering: "off"`). |
| mcp.ingress.host | string | `""` | Host for the `/mcp` path. Empty uses the first `ingress.hosts[0].host`. Set to a dedicated hostname (e.g. `mcp.example.com`) to separate MCP from the UI origin — add a matching `ingress.tls` entry for it. |
