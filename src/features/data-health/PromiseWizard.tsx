...
import { detectEventTimeColumn, DH_LABEL, DH_PRIMARY, eventTimeSupport, isSupportedEventTimeColumnType as isTemporalColumnType, suggestEventTimeEncoding } from "./lib";
...
function defaultForm(): FormState {
  return {
    name: "", description: "", sourceType: "table", databaseName: "", tableName: "", sourceQuery: "", eventTimeColumn: "", eventTimeEncoding: "native", eventTimeTimezone: "", rowFilter: "",
...
function formFromPromise(promise: DataHealthPromise): FormState {
  ...
  return {
    ...
    eventTimeColumn: promise.eventTimeColumn ?? "", eventTimeEncoding: promise.eventTimeEncoding ?? "auto", eventTimeTimezone: promise.eventTimeTimezone ?? "", rowFilter: promise.rowFilter ?? "", criticality: promise.criticality,
...
export function PromiseWizard({ open, onOpenChange, promise }: { open: boolean; onOpenChange: (open: boolean) => void; promise?: DataHealthPromise }) {
  ...
  const update = (patch: Partial<FormState>) => setForm((current) => {
    if (patch.eventTimeColumn !== undefined && patch.eventTimeColumn !== current.eventTimeColumn && patch.eventTimeEncoding === undefined) {
      ...
      return {
        ...
        eventTimeTimezone: column && eventTimeSupport(column.type) === "string" ? current.timezone : "",
      };
    }
    ...
    void getTableDetails(form.databaseName, form.tableName).then((details) => {
      ...
      setForm((current) => {
        ...
        const selectedColumnIsValid = details.columns.some((column) =>
          column.name === current.eventTimeColumn && eventTimeSupport(column.type) !== "unsupported");
  ...
  const selectedEventTimeType = columns.find((column) => column.name === form.eventTimeColumn)?.type ?? promise?.eventTimeType ?? "";
  const dateOnlyEventTime = /(^|\W)Date(?:32)?(?=\W|$)/.test(selectedEventTimeType) && !selectedEventTimeType.includes("DateTime");
  ...
  const canContinue = step === 0
    ...
    : step === 1
      ? checks.length > 0 && ruleDefinitionsValid && (!needsEventTime || Boolean(form.eventTimeColumn)) && (!form.eventTimeColumn || form.eventTimeEncoding !== "auto") && (form.frequency !== "cron" || Boolean(form.cronExpr.trim()))
      : true;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            {form.eventTimeEncoding === "auto" && <div className="rounded-xs border border-amber-500/40 bg-amber-500/10 p-4 text-[11px] text-amber-500"><p className="font-medium">This existing promise uses legacy event-time auto-detection.</p><p className="mt-1">Go back and confirm the stored encoding, Unix precision, and timezone before saving.</p></div>}
            {dateOnlyEventTime && form.frequency === "cron" && <div className="rounded-xs border border-amber-500/40 bg-amber-500/10 p-4 text-[11px] text-amber-500"><p className="font-medium">The selected event-time column has day-only precision.</p><p className="mt-1">Minute or hourly cron windows cannot be represented accurately by Date/Date32 values. Prefer a DateTime column or a daily cadence.</p></div>}
