...
function defaultForm(): FormState {
  return {
    ...
    completeness: false, completenessColumn: "", completenessPercent: 99.9, uniqueness: false, uniquenessColumns: [], duplicatePercent: 0,
    validity: false, validityPredicate: "", validityPercent: 99, schemaContract: true, allowAdditionalColumns: true,
    customMetric: false, customExpression: "", customOperator: "gte", customThreshold: 0, customUpperThreshold: 0,
  };
}
...
function formFromPromise(promise: DataHealthPromise): FormState {
  ...
  const completeness = findCheck(promise, "completeness");
  const uniqueness = findCheck(promise, "uniqueness");
  const validity = findCheck(promise, "validity");
  const schema = findCheck(promise, "schema_contract");
  const custom = findCheck(promise, "custom_metric");
  return {
    ...form,
    ...
    completeness: Boolean(completeness), completenessColumn: completeness?.type === "completeness" ? completeness.config.column : "", completenessPercent: completeness?.type === "completeness" ? completeness.config.minRatio * 100 : 99.9,
    uniqueness: Boolean(uniqueness), uniquenessColumns: uniqueness?.type === "uniqueness" ? uniqueness.config.columns : [], duplicatePercent: uniqueness?.type === "uniqueness" ? uniqueness.config.maxDuplicateRatio * 100 : 0,
    validity: Boolean(validity), validityPredicate: validity?.type === "validity" ? validity.config.predicate : "", validityPercent: validity?.type === "validity" ? validity.config.minRatio * 100 : 99,
    ...
    customMetric: Boolean(custom), customExpression: custom?.type === "custom_metric" ? custom.config.expression : "", customOperator: custom?.type === "custom_metric" ? custom.config.operator : "gte",
    customThreshold: custom?.type === "custom_metric" ? custom.config.threshold : 0, customUpperThreshold: custom?.type === "custom_metric" ? custom.config.upperThreshold ?? 0 : 0,
  };
}
...
export function PromiseWizard({ open, onOpenChange, promise }: { open: boolean; onOpenChange: (open: boolean) => void; promise?: DataHealthPromise }) {
  ...
  useEffect(() => {
    ...
    void getTableDetails(form.databaseName, form.tableName).then((details) => {
      ...
      if (!promise) {
        ...
        const keyColumn = details.columns.find((column) => /(^id$|_id$)/i.test(column.name))?.name ?? "";
        update({ eventTimeColumn: form.eventTimeColumn || timeColumn, completenessColumn: form.completenessColumn || keyColumn, uniquenessColumns: form.uniquenessColumns.length > 0 ? form.uniquenessColumns : keyColumn ? [keyColumn] : [] });
      }
  ...
  const checks = useMemo<DataHealthCheck[]>(() => {
    ...
    if (form.completeness && form.completenessColumn) result.push({ checkKey: `complete_${form.completenessColumn}`.slice(0, 64), name: `${form.completenessColumn} completeness`, type: "completeness", severity: "warning", enabled: true, config: { column: form.completenessColumn, minRatio: form.completenessPercent / 100 } });
    if (form.uniqueness && form.uniquenessColumns.length > 0) result.push({ checkKey: "key_uniqueness", name: "Key uniqueness", type: "uniqueness", severity: "critical", enabled: true, config: { columns: form.uniquenessColumns, maxDuplicateRatio: form.duplicatePercent / 100 } });
    if (form.validity && form.validityPredicate.trim()) result.push({ checkKey: "row_validity", name: "Business-rule validity", type: "validity", severity: "warning", enabled: true, config: { predicate: form.validityPredicate.trim(), minRatio: form.validityPercent / 100 } });
    ...
    if (form.customMetric && form.customExpression.trim()) result.push({ checkKey: "custom_metric", name: "Custom metric", type: "custom_metric", severity: "warning", enabled: true, config: { expression: form.customExpression.trim(), operator: form.customOperator, threshold: form.customThreshold, upperThreshold: form.customOperator === "between" ? form.customUpperThreshold : undefined } });
    return result;
  }, [form, columns]);
  ...
  const needsEventTime = form.freshness || form.rowCount || form.anomaly || form.completeness || form.uniqueness || form.validity;
  const canContinue = step === 0 ? Boolean((promise?.connectionId || activeConnectionId) && form.name.trim() && (form.sourceType === "table" ? form.databaseName && form.tableName : form.sourceQuery.trim()) && (!needsEventTime || form.eventTimeColumn)) : step === 1 ? checks.length > 0 && (form.frequency !== "cron" || Boolean(form.cronExpr.trim())) : true;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="flex max-h-[92vh] max-w-4xl flex-col overflow-hidden rounded-xs border-ink-500 bg-ink-100 p-0 text-paper"><DialogHeader className="border-b border-ink-500 px-6 py-5"><DialogTitle>{promise ? "Edit Data Health promise" : "Protect a dataset"}</DialogTitle><DialogDescription>Describe what healthy data means. CHouse will generate, schedule, and evaluate the monitor.</DialogDescription><div className="mt-4 flex gap-1">{STEPS.map((label, index) => <div key={label} className={cn("flex flex-1 items-center gap-2 border-t-2 pt-2", index <= step ? "border-brand text-paper" : "border-ink-500 text-paper-faint")}><span className="font-mono text-[9px]">0{index + 1}</span><span className="font-mono text-[10px] uppercase tracking-[0.12em]">{label}</span></div>)}</div></DialogHeader>
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
        ...
        {step === 1 && <div className="space-y-5"><div className="grid gap-4 md:grid-cols-3"><div><Label>Evaluation cadence</Label><Select value={form.frequency} onValueChange={(value) => update({ frequency: value as DataHealthFrequency })}><SelectTrigger className="mt-1 rounded-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="daily">Daily</SelectItem><SelectItem value="weekly">Weekly</SelectItem><SelectItem value="monthly">Monthly</SelectItem><SelectItem value="cron">Custom cron</SelectItem><SelectItem value="manual">Manual only</SelectItem></SelectContent></Select></div>{form.frequency !== "manual" && form.frequency !== "cron" && <div><Label>Business hour</Label><Input type="number" min={0} max={23} value={form.hour} onChange={(event) => update({ hour: Number(event.target.value) })} className="mt-1 rounded-xs" /></div>}{form.frequency === "cron" && <div><Label>Cron expression</Label><Input value={form.cronExpr} onChange={(event) => update({ cronExpr: event.target.value })} placeholder="0 8 * * *" className="mt-1 rounded-xs font-mono" /></div>}<div><Label>Business timezone</Label><Input value={form.timezone} onChange={(event) => update({ timezone: event.target.value })} className="mt-1 rounded-xs" /></div></div><div><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-brand" /><p className={DH_LABEL}>What must remain true?</p></div><div className="mt-3 grid gap-2 lg:grid-cols-2"><CheckToggle checked={form.freshness} onCheckedChange={(freshness) => update({ freshness })} title="Delivery freshness" description="Latest event must arrive within the promised delay."><div><Label>Maximum delay, minutes</Label><Input type="number" min={1} value={form.freshnessMinutes} onChange={(event) => update({ freshnessMinutes: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.rowCount} onCheckedChange={(rowCount) => update({ rowCount })} title="Row volume" description="Protect against empty, partial, or unexpectedly large windows."><div><Label>Minimum rows</Label><Input type="number" value={form.rowCountMin} onChange={(event) => update({ rowCountMin: event.target.value })} className="mt-1 rounded-xs" /></div><div><Label>Maximum rows</Label><Input type="number" value={form.rowCountMax} onChange={(event) => update({ rowCountMax: event.target.value })} placeholder="Optional" className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.anomaly} onCheckedChange={(anomaly) => update({ anomaly })} title="Learned volume range" description="Compare each window with a transparent robust baseline."><div><Label>Hard minimum while learning</Label><Input type="number" value={form.anomalyHardMin} onChange={(event) => update({ anomalyHardMin: event.target.value })} placeholder="Optional" className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.completeness} onCheckedChange={(completeness) => update({ completeness })} title="Column completeness" description="Require a field to be populated in most rows."><div><Label>Column</Label><Select value={form.completenessColumn || "none"} onValueChange={(value) => update({ completenessColumn: value === "none" ? "" : value })}><SelectTrigger className="mt-1 rounded-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Select column</SelectItem>{columns.map((column) => <SelectItem key={column.name} value={column.name}>{column.name}</SelectItem>)}</SelectContent></Select></div><div><Label>Minimum complete %</Label><Input type="number" min={0} max={100} step={0.1} value={form.completenessPercent} onChange={(event) => update({ completenessPercent: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.uniqueness} onCheckedChange={(uniqueness) => update({ uniqueness })} title="Key uniqueness" description="Detect duplicate business keys in each delivery window."><div><Label>Key column</Label><Select value={form.uniquenessColumns[0] ?? "none"} onValueChange={(value) => update({ uniquenessColumns: value === "none" ? [] : [value] })}><SelectTrigger className="mt-1 rounded-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Select key</SelectItem>{columns.map((column) => <SelectItem key={column.name} value={column.name}>{column.name}</SelectItem>)}</SelectContent></Select></div><div><Label>Maximum duplicates %</Label><Input type="number" min={0} max={100} step={0.01} value={form.duplicatePercent} onChange={(event) => update({ duplicatePercent: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.schemaContract} onCheckedChange={(schemaContract) => update({ schemaContract })} title="Schema contract" description="Detect removed or retyped columns."><label className="col-span-full flex items-center gap-2 text-[11px] text-paper-muted"><Checkbox checked={form.allowAdditionalColumns} onCheckedChange={(checked) => update({ allowAdditionalColumns: checked === true })} /> Allow new columns</label></CheckToggle><CheckToggle checked={form.validity} onCheckedChange={(validity) => update({ validity })} title="Business-rule validity" description="Require rows to satisfy a domain predicate."><div className="sm:col-span-2"><Label>Valid-row predicate</Label><Input value={form.validityPredicate} onChange={(event) => update({ validityPredicate: event.target.value })} placeholder="amount >= 0 AND status != 'invalid'" className="mt-1 rounded-xs font-mono" /></div><div><Label>Minimum valid %</Label><Input type="number" min={0} max={100} value={form.validityPercent} onChange={(event) => update({ validityPercent: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></CheckToggle><CheckToggle checked={form.customMetric} onCheckedChange={(customMetric) => update({ customMetric })} title="Custom scalar metric" description="Evaluate a domain-specific aggregate expression."><div className="sm:col-span-2"><Label>Aggregate expression</Label><Input value={form.customExpression} onChange={(event) => update({ customExpression: event.target.value })} placeholder="countIf(payment_state = 'failed')" className="mt-1 rounded-xs font-mono" /></div><div><Label>Operator</Label><Select value={form.customOperator} onValueChange={(value) => update({ customOperator: value as FormState["customOperator"] })}><SelectTrigger className="mt-1 rounded-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="gte">≥</SelectItem><SelectItem value="gt">&gt;</SelectItem><SelectItem value="lte">≤</SelectItem><SelectItem value="lt">&lt;</SelectItem><SelectItem value="eq">=</SelectItem><SelectItem value="between">Between</SelectItem></SelectContent></Select></div><div><Label>Threshold</Label><Input type="number" value={form.customThreshold} onChange={(event) => update({ customThreshold: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></CheckToggle></div></div><div className="grid gap-4 sm:grid-cols-3"><div><Label>Alert after breaches</Label><Input type="number" min={1} max={20} value={form.breachAfter} onChange={(event) => update({ breachAfter: Number(event.target.value) })} className="mt-1 rounded-xs" /></div><div><Label>Recover after passes</Label><Input type="number" min={1} max={20} value={form.recoverAfter} onChange={(event) => update({ recoverAfter: Number(event.target.value) })} className="mt-1 rounded-xs" /></div><div><Label>Grace period, minutes</Label><Input type="number" min={0} value={form.graceMinutes} onChange={(event) => update({ graceMinutes: Number(event.target.value) })} className="mt-1 rounded-xs" /></div></div><div><Label>Notify on incident transitions</Label><div className="mt-2 grid gap-2 sm:grid-cols-2">{channels.map((channel) => <label key={channel.id} className="flex items-center gap-2 rounded-xs border border-ink-500 p-2 text-[11px] text-paper-muted"><Checkbox checked={form.channelIds.includes(channel.id)} onCheckedChange={(checked) => update({ channelIds: checked === true ? [...form.channelIds, channel.id] : form.channelIds.filter((id) => id !== channel.id) })} />{channel.name}<span className="ml-auto font-mono text-[9px] uppercase text-paper-faint">{channel.type}</span></label>)}{channels.length === 0 && <p className="text-[11px] text-paper-faint">No enabled notification channels. Configure one under Admin → Alerting.</p>}</div></div></div>}
