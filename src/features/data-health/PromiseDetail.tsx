...
export function PromiseDetail({ id, onBack, onEdit }: { id: string; onBack: () => void; onEdit: () => void }) {
  ...
  const inspectCheck = async (checkKey: string) => {
    setDialog("diagnostic");
    setLoading(true);
    setError(undefined);
    try {
      setDiagnostic(await diagnoseDataHealthCheck(id, checkKey));
    } catch (caught) {
