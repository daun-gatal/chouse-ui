...
import { DH_LABEL, DH_PRIMARY, HealthBadge, formatHealthTime, formatMetric, formatSchedule } from "./lib";
...
export function PromiseDetail({ id, onBack, onEdit }: { id: string; onBack: () => void; onEdit: () => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {promise.eventTimeColumn && <Card className="rounded-xs border-ink-500 bg-ink-100 p-4"><p className={DH_LABEL}>Event-time normalization</p><p className="mt-2 font-mono text-[11px] text-paper">{promise.eventTimeColumn} · {promise.eventTimeEncoding.replaceAll("_", " ")} · {promise.eventTimeTimezone ?? "column / server timezone"} → UTC</p>{promise.eventTimeEncoding === "auto" && <p className="mt-1 text-[10px] text-amber-500">Legacy auto-detection needs confirmation in Edit promise.</p>}</Card>}
