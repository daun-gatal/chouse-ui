...
export default function Doctor() {
  ...
  const isDoctorOnboarding = new URLSearchParams(location.search).get("guide")?.startsWith("doctor-") === true;
  ...
  useEffect(() => {
    if (!isDoctorOnboarding && !reportId && !scan.isPending && reports.length > 0) {
      navigate(`/doctor/${reports[0].id}`, { replace: true });
    }
  }, [isDoctorOnboarding, reportId, reports, scan.isPending, navigate]);

  const showPicker = canRun && enabled !== false && models.length > 1;
  const canScan = canRun && enabled === true && !scan.isPending && !scopeEmpty;
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden bg-ink-50" data-onboarding-id="doctor-page">
      {/* Header */}
      <header className="flex-none border-b border-ink-500 px-6 pb-4 pt-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex w-full flex-wrap items-center justify-end gap-2 xl:w-auto">
            ...
            <div className="order-first flex w-full basis-full items-center justify-end gap-2 xl:order-none xl:w-auto xl:basis-auto">
              {canRun && (
                <button
                  type="button"
                  disabled={enabled !== true || scan.isPending}
                  onClick={() => setScheduleOpen(true)}
                  data-onboarding-id="doctor-schedule"
                  className="grid h-9 w-9 shrink-0 place-items-center rounded-xs border border-ink-500 bg-ink-200 text-paper-muted transition-colors hover:bg-ink-300 hover:text-paper disabled:text-paper-faint disabled:opacity-60"
                  title={enabled === true
                    ? "Scheduled scans (daily / weekly / monthly)"
                    : "An enabled AI provider is required before scheduling health checks"}
              ...
              <Button
                onClick={startScan}
                disabled={!canScan}
                className="relative h-9 min-w-0 flex-1 shrink-0 justify-center gap-2 rounded-xs bg-brand px-4 font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-50 hover:bg-brand-soft xl:w-[190px] xl:flex-none"
              >
                <span
                  data-onboarding-id="doctor-run"
                  className="pointer-events-none absolute inset-0"
                />
                {!canRun ? (
                  <><ShieldOff className="h-3.5 w-3.5" /> View-only reports</>
                ) : enabledQuery.isError ? (
                  <><AlertCircle className="h-3.5 w-3.5" /> Health check unavailable</>
                ) : enabled === undefined ? (
                  <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Preparing…</>
                ) : enabled === false ? (
                  <><Sparkles className="h-3.5 w-3.5" /> Configure AI first</>
                ) : scan.isPending ? (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden w-64 flex-none flex-col border-r border-ink-500 md:flex">
          <div
            className="flex-none border-b border-ink-500 px-3 py-2"
            data-onboarding-id="doctor-history"
          >
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-4 md:hidden" data-onboarding-id="doctor-history">
            {reports.length > 0 || scan.isPending ? (
              <select
                ...
                className="h-9 w-full rounded-xs border border-ink-500 bg-ink-200 px-2 text-[12px] text-paper focus:border-brand focus:outline-none"
              >
              ...
              <div className="flex h-9 items-center justify-between rounded-xs border border-ink-500 bg-ink-200 px-3">
                <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-paper-faint">
                  History
                </span>
                <span className="text-[11px] text-paper-muted">No reports</span>
              </div>
            )}
          </div>
          ...
          <div className="mx-auto max-w-3xl">
            <div
              className="mb-4 flex min-h-9 items-center justify-between gap-3 border-b border-ink-500 pb-3"
              data-onboarding-id="doctor-report"
            >
              <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-paper-faint">
                Report workspace
              </span>
              <span className="text-right font-mono text-[9px] uppercase tracking-[0.12em] text-paper-muted">
