...
describe("DataOps AI API", () => {
  it("invokes object-scoped structured capabilities", async () => {
    ...
    server.use(http.post("/api/ai/invoke", async ({ request }) => {
      const body = await request.json() as { capability: string; modelId?: string };
      seen.push(body.capability);
      seenModelIds.push(body.modelId);
    ...
    expect((await diagnoseScheduledRun("job-1", "run-1", { modelId: "model-1" })).likelyCause).toBe("Schema drift");
    ...
    // The page-selected model is forwarded when given, omitted otherwise.
    expect(seenModelIds).toEqual([undefined, "model-1", undefined]);
  });
});
