#!/bin/bash
#
# DinD Docker-Compose E2E for ADR 0011 (personal access tokens).
#
#   ./scripts/e2e-pat.sh
#
...
# then runs scripts/e2e-pat-check.py from INSIDE the compose network — published
# ports land on the DinD host, not on this pod, so localhost checks would
# falsely fail (see ADR 0011 E2E notes).
#
# Target environment: DOCKER_HOST=tcp://opencode-dind:2375
# (k8s service opencode-dind-apps under namespace opencode).
#
# Preconditions: docker client pointed at DinD, ports 5521/8124 free on the
# DinD host, no other chouse-ui-local stack running (sequential runs only).
#
set -euo pipefail

cd "$(dirname "$0")/.."

PROJECT="chouse-pat-e2e"
# The override drops the ./.config.yaml bind mount (unresolvable under DinD —
# see scripts/e2e-pat.override.yml) so the server boots from env + defaults.
...
echo "Running PAT checks from inside the compose network..."
...
set +e
docker run --rm -i --network "$NETWORK" "$SIDECAR_IMAGE" python3 - < scripts/e2e-pat-check.py
CHECK_STATUS=$?
set -e
