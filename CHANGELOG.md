...
- **Chouse AI in the monitoring tabs** — the same read-only engine surfaced where you're already looking, so a problem gets fixed without leaving the tab. Query Logs: "Optimize with Chouse AI" on any row → an optimized rewrite with the same result, a before→after `EXPLAIN` estimate, and "Open in Explorer". Errors: "Fix" on a `system.errors` row → cause / impact / ordered solutions. Parts: "Diagnose" on a part-log row → part-health read (merge pressure, too many parts, partition key). Backend `optimizeSingleQuery` / `diagnoseError` / `diagnoseParts` reuse the Fleet Doctor agent + a shared `SYSTEM_TABLE_REFERENCE`; endpoints gated by `ai:optimize`, advisory only — review before running.
